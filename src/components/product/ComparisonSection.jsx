...
export default function ComparisonSection() {
  const comparisonData = [
    { feature: "Price Model", integrit: "One-time", rev: "$0.25/min", descript: "$15/mo", manual: "Free (Your time)" },
    { feature: "Adobe Integration", integrit: true, rev: false, descript: false, manual: true },
    { feature: "Indian Languages", integrit: true, rev: "Limited", descript: "Limited", manual: true },
    { feature: "Phonetic Romanization", integrit: true, rev: false, descript: false, manual: true },
    { feature: "Generation Speed", integrit: "< 30s", rev: "Minutes", descript: "Minutes", manual: "Hours" },
  ];
  ...
  return (
    <section className="py-32 px-6 bg-bg-secondary border-y border-white/5">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <h2 className="font-display text-4xl md:text-5xl font-bold mb-4 text-white">Caption Integrit vs. The Rest</h2>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr>
                ...
                <th className="p-4 border-b-2 border-accent-primary bg-accent-primary/5 text-accent-primary font-bold text-lg w-1/4 rounded-t-xl">Caption Integrit</th>
              ...
              {comparisonData.map((row, idx) => (
                <tr key={idx} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-4 bg-accent-primary/5 border-x border-accent-primary/10">
                    {typeof row.integrit === 'boolean' ? (
                      row.integrit ? <Check className="w-5 h-5 text-accent-primary" /> : <X className="w-5 h-5 text-text-secondary" />
                    ) : (
                      <span className="font-bold text-white">{row.integrit}</span>
                    )}
                  </td>
